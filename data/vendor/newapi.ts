...
const vendor: VendorConfig = {
  ...
  models: [{ name: "GPT-Image 2", modelName: "gpt-image-2", type: "image", mode: ["text", "singleImage", "multiReference"] }],
};
...
const textRequest = (model: TextModel, think: boolean, thinkLevel: 0 | 1 | 2 | 3) => {
  throw new Error("newapi 供应商暂未支持文本模型");
};
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  // GPT-Image 1.5 负载高时会返回排队消息，自动重试
  const MAX_RETRIES = 6;
