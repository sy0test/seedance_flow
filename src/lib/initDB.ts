...
export default async (knex: Knex, forceInit: boolean = false): Promise<void> => {
  const tables: TableSchema[] = [
    ...
    // Seedance 项目配置
    {
      name: "seedance_project",
      builder: (table) => {
        table.increments("id").primary();
        table.integer("projectId").notNullable();
        table.text("visualStyle");
        table.text("targetMedium");
        table.text("directorVersion").defaultTo("pro");
    ...
    // Seedance 集数管理
    {
      name: "seedance_episode",
      builder: (table) => {
        table.increments("id").primary();
        table.integer("projectId").notNullable();
        table.text("episodeKey").notNullable();
        table.text("scriptContent");
  ...
  // ── 迁移：给已有 seedance_project 表加 directorVersion 列 ──
  const hasSeedanceProject = await knex.schema.hasTable("seedance_project");
  if (hasSeedanceProject) {
    const hasCol = await knex.schema.hasColumn("seedance_project", "directorVersion");
    if (!hasCol) {
      await knex.schema.alterTable("seedance_project", (table) => {
        table.text("directorVersion").defaultTo("pro");
      });
      console.log("[初始化数据库] seedance_project 已加 directorVersion 列");
    }
  }
};
