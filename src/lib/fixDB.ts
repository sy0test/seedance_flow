...
export default async (knex: Knex): Promise<void> => {
  ...
  // Seedance 数据统一：seedance_episode 增加 scriptId 关联 o_script
  await addColumn("seedance_episode", "scriptId", "integer");
  ...
  const advancedAgentList = [
    ...
    { key: "seedanceAgent:director", name: "Seedance Agent:导演", desc: "剧本分析 + 全阶段审核" },
    { key: "seedanceAgent:artDesigner", name: "Seedance Agent:服化道", desc: "角色/场景提示词设计" },
    { key: "seedanceAgent:storyboardArtist", name: "Seedance Agent:分镜师", desc: "Seedance 2.0 提示词编写" },
  ];
