...
export default router.post(
  ...
  async (req, res) => {
    ...
    // 清理 Seedance 管线关联数据
    const seedanceEps = await u.db("seedance_episode").whereIn("scriptId", ids).select("id", "projectId");
    if (seedanceEps.length) {
      const epIds = seedanceEps.map((e: any) => e.id);
      const projectIds = [...new Set(seedanceEps.map((e: any) => e.projectId))];
      await u.db("seedance_output").whereIn("episodeId", epIds).delete();
      await u.db("seedance_episode").whereIn("id", epIds).delete();
      // 清理 memories（isolcationKey 格式: {projectId}:seedanceAgent:{episodeId}）
      for (const pid of projectIds) {
        for (const eid of epIds) {
          await u.db("memories").where("isolationKey", `${pid}:seedanceAgent:${eid}`).delete();
        }
      }
    }
