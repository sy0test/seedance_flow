...
export async function runDecisionAI(ctx: AgentContext) {
  ...
  const skill = path.join(u.getPath(["skills", "director"]), "script_agent_decision.md");
...
function createSubAgent(parentCtx: AgentContext) {
  ...
  const run_sub_agent_storySkeleton = tool({
    ...
    execute: async ({ prompt }) => {
      const skill = path.join(u.getPath(["skills", "director"]), "script_execution_skeleton.md");
  ...
  const run_sub_agent_adaptationStrategy = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = path.join(u.getPath(["skills", "director"]), "script_execution_adaptation.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
  ...
  const run_sub_agent_script = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = path.join(u.getPath(["skills", "director"]), "script_execution_script.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
  ...
  const run_supervision_agent = tool({
    ...
    inputSchema: jsonSchema<{ prompt: string }>(promptInput),
    execute: async ({ prompt }) => {
      const skill = path.join(u.getPath(["skills", "director"]), "script_agent_supervision.md");
      const systemPrompt = await fs.promises.readFile(skill, "utf-8");
