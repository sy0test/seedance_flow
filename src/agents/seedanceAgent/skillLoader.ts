...
export function loadSkill(skillName: string, version: string = "pro"): LoadedSkill {
  const base = version === "ultra" ? "director_ultra" : "director_pro";
  const skillDir = u.getPath(["skills", base, skillName]);
  console.log(`[Seedance] loadSkill version=${version} base=${base} skill=${skillName} dir=${skillDir}`);
